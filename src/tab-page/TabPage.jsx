...
const TabPage = ({ intl, ...props }) => {
  ...
  if (courseStatus === 'denied') {
    ...
    if (redirectUrl) {
      return (<Navigate to={redirectUrl} replace key={redirectUrl} />);
    }
  }
  ...
  return (
    <>
      ...
      {(!['loading', 'loaded', 'denied', 'badGateway'].includes(courseStatus)) && (
        <p className="text-center py-5 mx-auto" style={{ maxWidth: '30em' }}>
          {/* {intl.formatMessage(messages.failure)} */}
          There seems to be a network issue. Please check your connection and try again not.
        </p>
      )}
